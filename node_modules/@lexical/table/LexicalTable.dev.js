...
function registerTablePlugin(editor, options) {
  ...
  const {
    ...
    hasFitNestedTables = extension.signal(false)
  } = options ?? {};
...
function $insertTableNodesIntoCells(nodes, selection, hasFitNestedTables) {
  ...
  const isMultiCellRangeSelection = lexical.$isRangeSelection(selection) && $isTableCellNode(selection.anchor.getNode()) && !selection.anchor.getNode().is(selection.focus.getNode());
  ...
  const focusNode = selection.focus.getNode();
  const parentCell = utils.$findMatchingParent(focusNode, $isTableCellNode);
  if (!parentCell) {
    return false;
  }
...
function $calculateCellInsets(cell) {
  const cellDOM = lexical.$getEditor().getElementByKey(cell.getKey());
  if (cellDOM === null) {
    return 0;
  }
